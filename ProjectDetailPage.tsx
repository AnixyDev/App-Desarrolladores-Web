...
const ProjectDetailPage: React.FC = () => {
    ...
    const handleAddTask = async (e: React.FormEvent) => {
        ...
        if (newTaskDescription.trim() && projectId) {
            try {
                ...
                addToast(`Error al añadir tarea: ${(error as Error).message}`, 'error');
            }
        }
    };
    ...
    const handleToggleTask = async (task: Task) => {
        ...
        try {
            ...
            if (newStatus === 'completed') {
             ...
             addToast(`Error al actualizar tarea: ${(error as Error).message}`, 'error');
        }
    };
    ...
    const confirmDelete = async () => {
        if (taskToDelete) {
            try {
                ...
                addToast(`Error al eliminar tarea: ${(error as Error).message}`, 'error');
            } finally {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            // Note: In a real app, this would upload to storage and get a URL before calling the store.
            // addProjectFile is simulated and local for now.
            addProjectFile({
    ...
    const confirmDeleteFile = () => {
        if (fileToDelete) {
            // Note: this is a local-only simulation for now.
