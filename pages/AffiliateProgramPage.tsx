            ...
            <Card>
                ...
                <CardContent className="flex flex-col sm:flex-row items-center gap-4">
                    <input
                        ...
                        value={referralLink}
                        readOnly
                        className="flex-1 w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm sm:text-sm bg-gray-800 text-gray-300"
                    />
                        ...
                        <table className="w-full text-left min-w-[600px]">
                            <thead className="border-b border-gray-800">
                                <tr>
                                    ...
                                    <th className="p-4 text-right">Comisión Generada</th>
                                </tr>
                            </thead>
                            <tbody>
                                {referrals.map(ref => (
                                    <tr key={ref.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                                        <td className="p-4 text-white font-semibold">{ref.name}</td>
                                        <td className="p-4 text-gray-300">{ref.join_date}</td>
                                        <td className="p-4">
                                            <span className={`px-2 py-1 text-xs rounded-full ${ref.status === 'Subscribed' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                                                {ref.status}
                                            </span>
                                        </td>
                                        <td className="p-4 text-right text-white font-semibold">{formatCurrency(ref.commission_cents)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
